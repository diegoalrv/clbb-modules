...
class Indicator():
    ...
    def load_resource(self, resource, environment, user, fields='', query_params=''):
        parquet_path = f'/usr/src/app/shared/zone_{self.zone}/data/{resource}.parquet'
        if self.cache and False and os.path.exists(parquet_path):
    ...
    def load_data(self):
        ...
        self.bus_stops, _, self.deleted_bus_stops = self.load_resource('busstop', environment_id, user['id'], 'name,residents')
        print(self.bus_stops.columns)
        print(self.bus_stops['project'].value_counts(dropna=False))
        print(f'------------------- load of bus stops took {time.perf_counter() - start:.2f}s')
        ...
        print(f'------------------- load of neighborhoods took {time.perf_counter() - start:.2f}s')
        ...
        print(f'------------------- load of blocks took {time.perf_counter() - start:.2f}s')
        print('blocks:', len(self.blocks))

        start = time.perf_counter()
        self.edges, _, _ = self.load_resource('street', environment_id, user['id'], 'length,src,dst', 'network=walk')
        print(f'------------------- load of street took {time.perf_counter() - start:.2f}s')
        print('edges:', len(self.edges))

        start = time.perf_counter()
        self.nodes, _, _ = self.load_resource('node', environment_id, user['id'])
        print(f'------------------- load of node took {time.perf_counter() - start:.2f}s')

        print(len(set(self.nodes['id'])))
        print(len(set(list(self.edges['src']) + list(self.edges['dst']))))
        ...
        print(f'------------------- 1 part took {time.perf_counter() - start:.2f}s')
        ...
        print(f'------------------- 2 part took {time.perf_counter() - start:.2f}s')

        start = time.perf_counter()
        self.area = self.load_area_of_interest()
        print('area:', len(self.area))
        print(f'------------------- 3 part took {time.perf_counter() - start:.2f}s')
    ...
    def execute_process(self):
        ...
        self.net.set_pois(
            ...
            maxdist=10_000_000,
            maxitems=num_pois,
            x_col=self.bus_stops.geometry.x.values,
            y_col=self.bus_stops.geometry.y.values,
        )
        ...
        bus_stops_update_project = self.bus_stops[['project', 'change_type']]
        print(bus_stops_update_project[bus_stops_update_project['project'].notna()])
        # bus_stops_update_project = bus_stops_update_project[bus_stops_update_project['change_type'] != 'Create']
        # print(bus_stops_update_project)
        bus_stops_update_project.reset_index(inplace=True)
        print(bus_stops_update_project)
        bus_stops_update_project.rename(columns={'id': 'bus_stop'}, inplace=True)
        print(bus_stops_update_project)
    ...
    def execute(self):
        try:
            ...
            try:
                ...
                start = time.perf_counter()
                self.set_border('box')
                print(f'set border took {time.perf_counter() - start:.2f}s')
                ...
                print(f'compute histogram took {time.perf_counter() - start:.2f}s')

                if not self.base and len(self.projects) > 0 and not self.base_indicator.empty:
                    start = time.perf_counter()
                    self.compute_differences()
                    print(f'compute differences took {time.perf_counter() - start:.2f}s')
            except Exception as e:
