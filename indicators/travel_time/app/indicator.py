...
class Indicator():
    ...
    def load_env_variables(self):
        ...
        self.x_target = os.getenv('x_target', None)
        self.y_target = os.getenv('y_target', None)

        self.x_target = -73.110594
        self.y_target = -36.714389
    ...
    def load_data(self):
        ...
        # self.target = gpd.GeoDataFrame(geometry=[Point(-73.110594, -36.714389)])
        self.target = gpd.GeoDataFrame(geometry=[Point(self.x_target, self.y_target)])
    ...
    def load_bus_shapes(self):
        path = f'/usr/src/app/shared/zone_{self.zone}/shape_edges.parquet'
    ...
    def execute_process(self):
        ...
        targets['id'] = self.net.get_node_ids(targets['geometry'].x, targets['geometry'].y)
        targets = pd.merge(targets, self.net.nodes_df, on='id')

        targets['distance_to_nearest_node'] = targets.apply(distance_between_points, axis=1)
